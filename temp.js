api.controller = function ($scope, $rootScope) {
  ...
  $rootScope.$on("triggerOpenCallForm4", function (event, data) {
    ...
    c.openCallForm();
	  ...
	  // Open call for accept
  $rootScope.$on("triggerOpenCallForm3", function (event, data) {
		
		if (data.triggerType == "acceptTrigger1") {
      console.log("Accept trigger received for call number:");
  ...
  c.getCalculatedCutOffTime = function () {
    ...
    ga.getXMLAnswer(function (response) {
      ...
      c.loading = false;
      c.openModal();
      c.earliestStartTime = new Date(cutOffTime); 
  ...
  c.openCallForm = function () {
    c.resetFormFields();
    c.formVisible = true;
		console.log("Starting to calculate cut off ")
    c.getCalculatedCutOffTime();
  };

	c.openCallForms = function () {
    c.resetFormFields();
    c.formVisible = true;
		c.reasonVisible=true;
		console.log("Starting to calculate cut off ")
  ...
  c.resetFormFields = function () {
    ...
    c.duration = "30";
    c.startTimes = [{ time: "", index: 0 }];
		console.log("reset form fields done");
  };
};
