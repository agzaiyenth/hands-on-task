  ...
  if (input) {
    ...
    if (input.parentSysId) {
      if (caseRecord.get(input.parentSysId)) {
        ...
        data.isAssigned = false;
        gs.info(
          "[Call Management] Record with parentSysId not found: " +
            input.parentSysId
        );
      }
    } else {
    ...
    while (gr.next()) {
      ...
      if (status2 == 2) {
        console.log(
          "u_temp_meeting_date is empty. Adding times from u_final_times."
        );
        ...
        if (gr.getValue("u_final_times")) {
          ...
          try {
            finalTimesArray.forEach(function (timeObj) {
              if (timeObj.time) {
                var convertedTime = timeZoneConverter.convertToUserTimezone(
                  timeObj.time
                );
        ...
        var convertedTime =
          timeZoneConverter.convertToUserTimezone(tempMeetingDate);
        startTimes.push(convertedTime);
      }
      ...
      var callData = {
        ...
        startTimes:
          startTimes ||
          "You have to Accept the request and provide convenient start times.",
        status: status,
        stateread: stateread,
        statusColor: color,
    ...
    if (input.callNumber && input.cancelReason) {
      ...
      if (cancelReason) {
        ...
        if (!isCommented) {
          handleCallStates.addAComment(
            parentSysId,
            "Call [" + callNumber + "]  has been cancelled."
          );
          cancelReason = "";
          isCommented = true;
        }
